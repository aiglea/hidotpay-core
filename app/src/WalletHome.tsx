...
import { WALLET_NETWORKS, walletNetwork, type WalletNetwork } from './product-networks';
import { allocateDepositAddress, getWalletSnapshot, getWalletTransactions, requestWithdrawal, submitInternalTransfer, type DepositAddress, type WalletSnapshot, type WalletTransaction } from './wallet-api';

type WalletHomeProps = { apiBaseUrl: string; getAccessToken: () => Promise<string | undefined>; onSignOut: () => void; username?: string };
type RequestStatus = 'idle' | 'loading' | 'ready' | 'error';
type Screen = 'home' | 'topup' | 'topup-confirmation' | 'history' | 'send' | 'send-confirm' | 'send-done' | 'withdraw';
const TRANSFERABLE_ASSETS = ['USDT', 'USDC', 'BTC', 'XRP'] as const;
type Network = WalletNetwork['id'];
...
function formatAtoms(atoms: string, decimals: number): string {
  ...
  const value = BigInt(atoms); const whole = value / base; const fraction = (value % base).toString().padStart(decimals, '0').replace(/0+$/, '');
  return fraction ? `${whole.toLocaleString()}.${fraction}` : whole.toLocaleString();
}
function assetDecimals(assetCode: string): number { return assetCode === 'BTC' ? 8 : 6; }
function amountToAtoms(value: string, decimals: number): string {
  const trimmed = value.trim();
  if (!new RegExp(`^\\d+(\\.\\d{1,${decimals}})?$`).test(trimmed)) throw new Error(`請輸入有效的金額，最多 ${decimals} 位小數。`);
  const [whole, fraction = ''] = trimmed.split('.');
  const atoms = BigInt(whole) * (10n ** BigInt(decimals)) + BigInt(fraction.padEnd(decimals, '0'));
  if (atoms <= 0n) throw new Error('轉帳金額必須大於 0。');
  return atoms.toString();
}
function transactionLabel(type: string): string { return type === 'internal_transfer' ? '站內轉帳' : type === 'deposit_credit' || type === 'deposit_confirmed' || type === 'deposit' ? '充值入帳' : type === 'withdrawal' ? '提領' : '錢包異動'; }
...
export function WalletHome({ apiBaseUrl, getAccessToken, onSignOut, username }: WalletHomeProps) {
  ...
  const [network, setNetwork] = useState<Network>('ethereum-sepolia');
  ...
  const [withdrawNetwork, setWithdrawNetwork] = useState<Network>('ethereum-sepolia');
  ...
  const transferableAssets = useMemo(() => {
    const held = wallet?.balances.map((balance) => balance.assetCode) ?? [];
    return TRANSFERABLE_ASSETS.filter((asset) => held.includes(asset) || asset === 'USDT');
  }, [wallet]);
  ...
  const reviewTransfer = () => {
    ...
    try {
      ...
      amountToAtoms(transferAmount, assetDecimals(transferAsset));
  ...
  const confirmTransfer = async () => {
    ...
    try {
      const result = await submitInternalTransfer({
        ...
        amountAtoms: amountToAtoms(transferAmount, assetDecimals(transferAsset)),
        apiBaseUrl,
        assetCode: transferAsset,
  ...
  const openWithdraw = () => { setMessage(undefined); setScreen('withdraw'); };
  const confirmWithdrawal = async () => {
    if (!configured) return;
    setWithdrawBusy(true); setMessage(undefined);
    try {
      await requestWithdrawal({
        accessToken: await fetchToken(),
        amountAtoms: amountToAtoms(withdrawAmount, assetDecimals(withdrawAsset)),
        apiBaseUrl,
    ...
    } catch (reason) { setMessage(messageFrom(reason, '提領尚未開放。沒有正式簽名器前，不能把資產轉出平台。')); }
  ...
  if (screen === 'send') return <SendScreen amount={transferAmount} asset={transferAsset} assets={transferableAssets} configured={configured} message={message} recipient={recipientWalletId} walletId={wallet?.accountId} onAmount={setTransferAmount} onAsset={setTransferAsset} onBack={returnHome} onRecipient={setRecipientWalletId} onReview={reviewTransfer} />;
  if (screen === 'send-confirm') return <SendConfirmScreen amount={transferAmount} asset={transferAsset} busy={transferBusy} message={message} recipient={recipientWalletId.trim()} onBack={() => { setMessage(undefined); setScreen('send'); }} onConfirm={() => void confirmTransfer()} />;
  if (screen === 'send-done' && transferId) return <SendDoneScreen amount={transferAmount} asset={transferAsset} recipient={recipientWalletId.trim()} transferId={transferId} onDone={returnHome} />;
  if (screen === 'withdraw') return <WithdrawScreen address={withdrawAddress} amount={withdrawAmount} asset={withdrawAsset} assets={[...TRANSFERABLE_ASSETS]} busy={withdrawBusy} configured={configured} message={message} network={withdrawNetwork} onAddress={setWithdrawAddress} onAmount={setWithdrawAmount} onAsset={(code) => { setWithdrawAsset(code); const next = WALLET_NETWORKS.find((item) => item.assetCode === code); if (next) setWithdrawNetwork(next.id); }} onBack={returnHome} onNetwork={setWithdrawNetwork} onSubmit={() => void confirmWithdrawal()} />;
  ...
  return <View style={styles.page}><ScrollView contentContainerStyle={styles.scroll} showsVerticalScrollIndicator={false}>
    ...
    <View style={styles.balanceArea}><Text style={styles.balanceLabel}>總資產</Text>{status === 'loading' ? <ActivityIndicator color={figmaWallet.colors.black} /> : null}{status === 'error' ? <Text style={styles.error}>{message}</Text> : null}{configured && status === 'ready' && wallet?.balances.map((balance) => <Text key={balance.assetCode} style={styles.balanceValue}>{formatAtoms(balance.balanceAtoms, assetDecimals(balance.assetCode))} <Text style={styles.balanceUnit}>{balance.assetCode}</Text></Text>)}{configured && status === 'ready' && !wallet?.balances.length ? <Text style={styles.balanceEmpty}>尚無已入帳資產。這是測試網錢包，把對應測試網資產轉到你的充值地址後，確認完成才會入帳。</Text> : null}{!configured ? <Text style={styles.balanceEmpty}>錢包 API 尚未啟用，沒有顯示測試餘額。</Text> : null}</View>
    ...
    <View style={styles.actionRow}><Pressable accessibilityLabel="前往多鏈充值" accessibilityRole="button" onPress={() => setScreen('topup')} style={({ pressed }) => [styles.primaryAction, pressed && figmaWalletShared.pressed]}><Text style={styles.actionIcon}>↓</Text><Text style={styles.primaryActionText}>充值</Text></Pressable><Pressable accessibilityLabel="前往站內轉帳" accessibilityRole="button" onPress={openSend} style={({ pressed }) => [styles.secondaryAction, pressed && figmaWalletShared.pressed]}><Text style={styles.secondaryIcon}>↗</Text><Text style={styles.secondaryActionText}>轉帳</Text></Pressable><Pressable accessibilityLabel="前往外部提領" accessibilityRole="button" onPress={openWithdraw} style={({ pressed }) => [styles.withdrawAction, pressed && figmaWalletShared.pressed]}><Text style={styles.withdrawIcon}>↑</Text><Text style={styles.withdrawActionText}>提領</Text></Pressable></View>
    <View style={styles.purpleBanner}><Text style={styles.bannerTitle}>測試網充值，等待入帳</Text><Text style={styles.bannerBody}>第一版 15 條鏈都可取得真實地址。已驗證測試資產的網路會在確認後入帳；其餘只給地址、不會入帳。主網資產一律拒絕。</Text><Pressable accessibilityRole="button" onPress={() => setScreen('topup')}><Text style={styles.bannerLink}>開始充值　→</Text></Pressable></View>
    ...
    {configured && historyStatus === 'ready' && !history?.transactions.length ? <Text style={styles.notice}>尚無交易紀錄。等待第一筆測試網充值確認入帳後，會在這裡顯示。</Text> : null}
...
function TopUpConfirmation({ address, copied, onBack, onCopy, onDone }: { address: DepositAddress; copied: boolean; onBack: () => void; onCopy: () => void; onDone: () => void }) {
  const selected = walletNetwork(address.network);
  return <View style={styles.purplePage}><ScrollView contentContainerStyle={styles.topupScroll} showsVerticalScrollIndicator={false}><TopBar title="充值地址" onBack={onBack} /><View style={styles.sheet}><View style={styles.successCircle}><Text style={styles.successTick}>✓</Text></View><Text style={styles.sheetTitle}>地址已準備好</Text><Text style={styles.sheetBody}>僅限透過 <Text style={styles.emphasis}>{selected?.name ?? address.network}</Text> 轉入 {selected?.assetCode ?? '指定資產'}。主網或網路不一致不會入帳。{selected && !selected.creditLive ? ' 此測試網目前只提供地址，轉入不會入帳。' : ''}</Text><View style={styles.addressCard}><Text style={styles.addressLabel}>{(selected?.name ?? address.network).toUpperCase()} · {selected?.assetCode ?? 'TEST'}</Text><Text selectable style={styles.addressText}>{address.address}</Text><Pressable accessibilityRole="button" onPress={onCopy} style={({ pressed }) => [styles.copyButton, pressed && figmaWalletShared.pressed]}><Text style={styles.copyText}>{copied ? '已複製' : '複製地址'}</Text></Pressable></View><View style={styles.warningCard}><Text style={styles.warningTitle}>{selected?.creditLive ? '等待第一筆測試網入帳' : '此鏈地址已配置，入帳尚未開放'}</Text><Text style={styles.warningBody}>{selected?.creditLive ? '轉入後需等待鏈上確認，掃描器入帳後餘額才會更新。這不是主網，也不能保管真實資產。' : selected?.description ?? '此測試網沒有已驗證的公開測試資產，轉入不會入帳。'}</Text></View><Pressable accessibilityRole="button" onPress={onDone} style={({ pressed }) => [figmaWalletShared.button, pressed && figmaWalletShared.pressed]}><Text style={figmaWalletShared.buttonText}>我已了解</Text></Pressable></View></ScrollView></View>;
}

function SendScreen({ amount, asset, assets, configured, message, recipient, walletId, onAmount, onAsset, onBack, onRecipient, onReview }: { amount: string; asset: string; assets: readonly string[]; configured: boolean; message?: string; recipient: string; walletId?: string; onAmount: (value: string) => void; onAsset: (value: string) => void; onBack: () => void; onRecipient: (value: string) => void; onReview: () => void }) {
  return <View style={styles.purplePage}><ScrollView contentContainerStyle={styles.topupScroll} showsVerticalScrollIndicator={false}><TopBar title="站內轉帳" onBack={onBack} /><View style={styles.sheet}><Text style={styles.sheetEyebrow}>轉給 hidotpay 用戶</Text><Text style={styles.sheetTitle}>即時到帳，不上鏈</Text><Text style={styles.sheetBody}>輸入對方的錢包編號與已入帳資產金額。這是平台內轉帳，不會產生鏈上手續費，也不會觸發外部提領。</Text>{walletId ? <View style={styles.addressCard}><Text style={styles.addressLabel}>你的錢包編號</Text><Text selectable style={styles.addressText}>{walletId}</Text></View> : null}<Text style={styles.fieldLabel}>選擇資產</Text><View style={styles.assetRow}>{assets.map((code) => <Pressable key={code} accessibilityLabel={`選擇 ${code}`} accessibilityRole="button" onPress={() => onAsset(code)} style={[styles.assetChip, asset === code && styles.assetChipActive]}><Text style={[styles.assetChipText, asset === code && styles.assetChipTextActive]}>{code}</Text></Pressable>)}</View><Text style={styles.fieldLabel}>收款錢包編號</Text><TextInput accessibilityLabel="收款錢包編號" autoCapitalize="none" autoCorrect={false} onChangeText={onRecipient} placeholder="00000000-0000-4000-8000-000000000000" placeholderTextColor={figmaWallet.colors.muted} style={styles.input} value={recipient} /><Text style={styles.fieldLabel}>金額（{asset}）</Text><TextInput accessibilityLabel="轉帳金額" autoCapitalize="none" keyboardType="decimal-pad" onChangeText={onAmount} placeholder="0.00" placeholderTextColor={figmaWallet.colors.muted} style={styles.input} value={amount} />{message ? <Text style={styles.errorDark}>{message}</Text> : null}<Pressable accessibilityRole="button" disabled={!configured} onPress={onReview} style={({ pressed }) => [figmaWalletShared.button, !configured && styles.disabled, pressed && figmaWalletShared.pressed]}><Text style={figmaWalletShared.buttonText}>{configured ? '檢查轉帳內容' : '轉帳服務尚未啟用'}</Text></Pressable></View></ScrollView></View>;
}

function SendConfirmScreen({ amount, asset, busy, message, recipient, onBack, onConfirm }: { amount: string; asset: string; busy: boolean; message?: string; recipient: string; onBack: () => void; onConfirm: () => void }) {
  return <View style={styles.purplePage}><ScrollView contentContainerStyle={styles.topupScroll} showsVerticalScrollIndicator={false}><TopBar title="確認轉帳" onBack={onBack} /><View style={styles.sheet}><Text style={styles.sheetTitle}>請再確認一次</Text><Text style={styles.sheetBody}>送出後會立刻從你的可用餘額扣款，並記入對方錢包。請確認編號與金額無誤。</Text><View style={styles.addressCard}><Text style={styles.addressLabel}>收款錢包</Text><Text selectable style={styles.addressText}>{recipient}</Text><Text style={styles.addressLabel}>金額</Text><Text style={styles.confirmAmount}>{amount} {asset}</Text></View>{message ? <Text style={styles.errorDark}>{message}</Text> : null}<Pressable accessibilityRole="button" disabled={busy} onPress={onConfirm} style={({ pressed }) => [figmaWalletShared.button, busy && styles.disabled, pressed && figmaWalletShared.pressed]}>{busy ? <ActivityIndicator color={figmaWallet.colors.black} /> : <Text style={figmaWalletShared.buttonText}>確認轉出</Text>}</Pressable></View></ScrollView></View>;
}

function SendDoneScreen({ amount, asset, recipient, transferId, onDone }: { amount: string; asset: string; recipient: string; transferId: string; onDone: () => void }) {
  return <View style={styles.purplePage}><ScrollView contentContainerStyle={styles.topupScroll} showsVerticalScrollIndicator={false}><TopBar title="轉帳完成" onBack={onDone} /><View style={styles.sheet}><View style={styles.successCircle}><Text style={styles.successTick}>✓</Text></View><Text style={styles.sheetTitle}>已轉給對方</Text><Text style={styles.sheetBody}>{amount} {asset} 已記入對方的 hidotpay 錢包，無需等待鏈上確認。</Text><View style={styles.addressCard}><Text style={styles.addressLabel}>收款錢包</Text><Text selectable style={styles.addressText}>{recipient}</Text><Text style={styles.addressLabel}>轉帳編號</Text><Text selectable style={styles.addressText}>{transferId}</Text></View><Pressable accessibilityRole="button" onPress={onDone} style={({ pressed }) => [figmaWalletShared.button, pressed && figmaWalletShared.pressed]}><Text style={figmaWalletShared.buttonText}>返回錢包</Text></Pressable></View></ScrollView></View>;
}

function WithdrawScreen({ address, amount, asset, assets, busy, configured, message, network, onAddress, onAmount, onAsset, onBack, onNetwork, onSubmit }: { address: string; amount: string; asset: string; assets: readonly string[]; busy: boolean; configured: boolean; message?: string; network: Network; onAddress: (value: string) => void; onAmount: (value: string) => void; onAsset: (value: string) => void; onBack: () => void; onNetwork: (value: Network) => void; onSubmit: () => void }) {
  return <View style={styles.purplePage}><ScrollView contentContainerStyle={styles.topupScroll} showsVerticalScrollIndicator={false}><TopBar title="提領" onBack={onBack} /><View style={styles.sheet}><Text style={styles.sheetEyebrow}>轉出平台</Text><Text style={styles.sheetTitle}>外部提領</Text><Text style={styles.sheetBody}>提領需要正式簽名器與雙人覆核。送出後帳本會拒絕這筆請求，資產不會凍結。</Text><Text style={styles.fieldLabel}>選擇資產</Text><View style={styles.assetRow}>{assets.map((code) => <Pressable key={code} accessibilityLabel={`提領 ${code}`} accessibilityRole="button" onPress={() => onAsset(code)} style={[styles.assetChip, asset === code && styles.assetChipActive]}><Text style={[styles.assetChipText, asset === code && styles.assetChipTextActive]}>{code}</Text></Pressable>)}</View><Text style={styles.fieldLabel}>網路</Text>{WALLET_NETWORKS.filter((item) => item.assetCode === asset).map((item) => <Pressable key={item.id} accessibilityRole="button" onPress={() => onNetwork(item.id)} style={({ pressed }) => [styles.networkCard, network === item.id && styles.networkCardActive, pressed && figmaWalletShared.pressed]}><View style={styles.networkDetails}><Text style={styles.networkName}>{item.name}</Text><Text style={styles.networkDescription}>{item.description}</Text></View><Text style={styles.choiceMark}>{network === item.id ? '✓' : ''}</Text></Pressable>)}<Text style={styles.fieldLabel}>目標地址</Text><TextInput accessibilityLabel="提領目標地址" autoCapitalize="none" autoCorrect={false} onChangeText={onAddress} placeholder="測試網地址" placeholderTextColor={figmaWallet.colors.muted} style={styles.input} value={address} /><Text style={styles.fieldLabel}>金額（{asset}）</Text><TextInput accessibilityLabel="提領金額" autoCapitalize="none" keyboardType="decimal-pad" onChangeText={onAmount} placeholder="0.00" placeholderTextColor={figmaWallet.colors.muted} style={styles.input} value={amount} />{message ? <Text style={styles.errorDark}>{message}</Text> : null}<Pressable accessibilityRole="button" disabled={!configured || busy} onPress={onSubmit} style={({ pressed }) => [figmaWalletShared.button, (!configured || busy) && styles.disabled, pressed && figmaWalletShared.pressed]}>{busy ? <ActivityIndicator color={figmaWallet.colors.black} /> : <Text style={figmaWalletShared.buttonText}>{configured ? '確認提領條件' : '提領服務尚未啟用'}</Text>}</Pressable></View></ScrollView></View>;
}
