...
type Network = 'ethereum-sepolia' | 'tron-shasta';
...
export function WalletHome({ apiBaseUrl, getAccessToken, onSignOut, username }: WalletHomeProps) {
  ...
  return <View style={styles.page}><ScrollView contentContainerStyle={styles.scroll} showsVerticalScrollIndicator={false}>
    ...
    <View style={styles.purpleBanner}><Text style={styles.bannerTitle}>測試網充值，等待入帳</Text><Text style={styles.bannerBody}>Ethereum Sepolia 與 TRON Shasta 可取得你的專屬地址。轉入測試 USDT 後，掃描器確認才會記入餘額。</Text><Pressable accessibilityRole="button" onPress={() => setScreen('topup')}><Text style={styles.bannerLink}>開始充值　→</Text></Pressable></View>
