...
export type OutboxPublisherHealthServer = {
  markReady(): void;
  markStopping(): void;
  markUnready(): void;
  server: Server;
...
/** This private probe never exposes database, broker, or financial data. */
export function createOutboxPublisherHealthServer(): OutboxPublisherHealthServer {
  let ready = false;
  let stopping = false;
  ...
  return {
    markReady: () => { if (!stopping) ready = true; },
