...
1. **錢包 App**：登入後已具備餘額、獨立充值地址與站內轉帳的受保護介面；未設定 API 時明確顯示保護狀態，不產生假資料。交易紀錄、P2P 介面及實際已授權 API 端到端驗收仍未完成。
...
下一個可交付項目是 **受保護 API 的 Logto Resource、HTTPS／CORS allowlist 正式化與使用者交易紀錄 API**；之後才能以真實登入帳戶進行錢包介面的端到端驗收。同時保留 P1.2 的私有正式網路、HTTPS、備份還原與管理者驗收作為資金上線前門檻。兩者都不會碰觸私鑰或開啟主網提現。
